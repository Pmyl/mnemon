...
| Step | Goal | Reviewable Outcome | Dependencies |
| --- | --- | --- | --- |
| **Step 7 — Memory Details view with audio controls (stub)** ✅ | Render Details view as slide-up panel, share components for feelings/notes, and wire a stubbed audio controller. | Clicking bottom title bar slides hero up to reveal details below; clicking title bar again slides back down. Audio player stub with play/pause button (no actual audio yet). | Step 6 |
...
| rev. 6 | Step 7 complete | Implemented Memory Details as slide-up panel (not separate route): (1) **Slide-up interaction** - Hero slides up to reveal details below when clicking bottom title bar area, (2) **Title bar as header** - When details open, only title bar remains visible at top, acting as both header and click target to close, (3) **Two click zones** - Upper area opens Add flow, bottom title bar toggles details, (4) **Auto-cycle pauses** - Slideshow pauses when details are open, resumes when closed, (5) **MemoryDetails component** - Shows feelings chips, finished date, and notes in scrollable area, (6) **Stubbed audio player** - Play/pause button toggles state (no actual audio), progress bar placeholder, only shown when theme_music_local_uri exists, (7) **Constants** - Added DETAILS_TRANSITION_MS (400ms) and TITLE_BAR_HEIGHT_PX (80px) to constants.rs, (8) **Chevron indicator** - Visual hint in title bar rotates when details are open/closed. No routing needed - all in single page with CSS transforms. |
| rev. 7 | _Pending_ | _TBD_ |
